...
layout: post
published: false
title: 『 Spark 』7. 深入研究 spark 运行原理之 job, stage, task
...
### spark logic
res = sc.wholeTextFiles('hdfs://10.21.208.21:8020/user/mercury/names', minPartitions=40)  \
        .map(map_extract) \
        .flatMap(lambda x: x) \
...
### result displaying
data = pd.DataFrame.from_records(res, columns=['year', 'birth']).sort(columns=['year'], ascending=True)
ax = data.plot(x=['year'], y=['birth'], figsize=(20, 6), title='US Baby Birth Data from 1897 to 2014', linewidth=3)
